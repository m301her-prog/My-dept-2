export const translations = {
  ar: {
    // Auth - Modern Standard Arabic
    login: 'تسجيل الدخول',
    register: 'إنشاء حساب',
    email: 'البريد الإلكتروني',
    password: 'كلمة المرور',
    name: 'الاسم',
    phone: 'رقم الهاتف',
    confirmPassword: 'تأكيد كلمة المرور',
    noAccount: 'ليس لديك حساب؟ سجل الآن',
    hasAccount: 'لديك حساب؟ سجل دخولك',
    loginTitle: 'تسجيل الدخول إلى حسابك',
    registerTitle: 'إنشاء حساب جديد',
    logout: 'تسجيل الخروج',
    forgotPassword: 'نسيت كلمة المرور؟',

    // Common Phrases
    welcome: 'مرحباً',
    hello: 'مرحباً',
    howAreYou: 'كيف حالك؟',
    goodMorning: 'صباح الخير',
    goodEvening: 'مساء الخير',
    thankYou: 'شكراً لك',
    noProblem: 'لا مشكلة',
    yes: 'نعم',
    no: 'لا',
    ok: 'حسناً',
    wait: 'انتظر رجاءً',
    done: 'تم',
    loading: 'جاري التحميل...',
    please: 'رجاءً',
    sorry: 'عذراً',

    // Navigation
    home: 'الرئيسية',
    debts: 'الديون',
    settings: 'الإعدادات',
    admin: 'لوحة التحكم',
    back: 'رجوع',
    next: 'التالي',
    previous: 'السابق',
    close: 'إغلاق',

    // Debt Types - Modern Standard Arabic
    debtOwedToMe: 'الديون المستحقة لي',
    debtIOwe: 'الديون المتوجبة عليّ',
    owedToMe: 'لي',
    iOwe: 'عليّ',
    moneyOwed: 'المبالغ المستحقة لي',
    moneyIOwe: 'المبالغ المتوجبة عليّ',

    // Debt Form
    addDebt: 'إضافة دين',
    editDebt: 'تعديل الدين',
    debtType: 'نوع الدين',
    personName: 'اسم الشخص',
    amount: 'المبلغ',
    currency: 'العملة',
    dueDate: 'تاريخ الاستحقاق',
    notes: 'ملاحظات',
    save: 'حفظ',
    cancel: 'إلغاء',
    delete: 'حذف',
    confirm: 'تأكيد',

    // Debt Status
    pending: 'قيد الانتظار',
    paid: 'مسدد',
    partiallyPaid: 'مسدد جزئياً',
    overdue: 'متأخر',
    status: 'الحالة',

    // Statistics
    totalOwedToMe: 'إجمالي المستحق لي',
    totalIOwe: 'إجمالي المتوجب عليّ',
    paidDebts: 'الديون المسددة',
    pendingDebts: 'الديون المعلقة',
    statistics: 'الإحصائيات',
    recentActivity: 'النشاط الأخير',
    totalDebts: 'مجموع الديون',

    // Person Management
    people: 'الأشخاص',
    addPerson: 'إضافة شخص',
    editPerson: 'تعديل بيانات الشخص',
    deletePerson: 'حذف الشخص',
    personDetails: 'تفاصيل الشخص',
    contactPerson: 'التواصل مع الشخص',

    // Settings
    appearance: 'المظهر',
    darkMode: 'الوضع الداكن',
    lightMode: 'الوضع الفاتح',
    language: 'اللغة',
    notifications: 'الإشعارات',
    whatsappReminder: 'التذكير عبر واتساب',
    enableNotifications: 'تفعيل الإشعارات',
    enableWhatsapp: 'تفعيل التذكير عبر واتساب',
    clearData: 'مسح جميع البيانات',
    accountSettings: 'إعدادات الحساب',

    // Languages
    arabic: 'العربية',
    french: 'الفرنسية',
    english: 'الإنجليزية',

    // Admin
    users: 'المستخدمون',
    userEmail: 'البريد الإلكتروني',
    registrationDate: 'تاريخ التسجيل',
    activate: 'تفعيل',
    deactivate: 'تعطيل',
    deleteUser: 'حذف المستخدم',
    totalUsers: 'إجمالي المستخدمين',
    activeUsers: 'المستخدمون النشطون',
    manageUsers: 'إدارة المستخدمين',

    // Messages - Modern Standard Arabic
    debtAdded: 'تمت إضافة الدين بنجاح',
    debtUpdated: 'تم تحديث الدين',
    debtDeleted: 'تم حذف الدين',
    loginSuccess: 'تم تسجيل الدخول بنجاح',
    logoutSuccess: 'تم تسجيل الخروج بنجاح',
    registerSuccess: 'تم إنشاء الحساب بنجاح',
    error: 'خطأ',
    success: 'نجاح',
    confirmDelete: 'هل أنت متأكد من الحذف؟ لا يمكن التراجع عن هذا الإجراء',
    noDebts: 'لا توجد ديون',
    noUsers: 'لا يوجد مستخدمون',
    loadingData: 'جاري التحميل...',
    savingData: 'جاري الحفظ...',
    networkError: 'خطأ في الاتصال - تحقق من شبكتك',
    invalidCredentials: 'بيانات الدخول غير صحيحة',
    emailExists: 'البريد الإلكتروني مسجل مسبقاً',

    // Currency
    dzd: 'دينار جزائري',
    eur: 'يورو',
    usd: 'دولار أمريكي',

    // Notifications
    paymentReminder: 'تذكير بالسداد',
    dueToday: 'موعد الاستحقاق اليوم',
    dueTomorrow: 'موعد الاستحقاق غداً',
    overdueNotice: 'دين متأخر',
    notificationSent: 'تم إرسال الإشعار',

    // WhatsApp Messages
    whatsappReminderTitle: 'تذكير بدين',
    whatsappGreeting: 'السلام عليكم،',
    whatsappBody: 'أذكركم بالدين التالي:',
    whatsappClosing: 'شكراً لتعاونكم',

    // Dates
    today: 'اليوم',
    tomorrow: 'غداً',
    yesterday: 'أمس',
    thisWeek: 'هذا الأسبوع',
    thisMonth: 'هذا الشهر',
    daysLeft: 'يوم متبقي',
    daysOverdue: 'يوم متأخر',

    // Actions
    call: 'اتصل',
    sendMessage: 'إرسال رسالة',
    markAsPaid: 'تعليم كمسدد',
    markAsPending: 'تعليم كقيد الانتظار',
    editAmount: 'تعديل المبلغ',
    addNote: 'إضافة ملاحظة',
    viewDetails: 'عرض التفاصيل',
    share: 'مشاركة',

    // Filter & Search
    search: 'بحث',
    searchPlaceholder: 'البحث باسم الشخص...',
    filter: 'تصفية',
    filterByType: 'تصفية حسب النوع',
    filterByStatus: 'تصفية حسب الحالة',
    sortBy: 'ترتيب حسب',
    sortNewest: 'الأحدث',
    sortOldest: 'الأقدم',
    sortAmount: 'حسب المبلغ',
    sortDueDate: 'حسب تاريخ الاستحقاق',

    // Misc
    version: 'الإصدار',
    appInfo: 'حول التطبيق',
    contactSupport: 'الدعم الفني',
    privacyPolicy: 'سياسة الخصوصية',
    termsOfService: 'شروط الاستخدام',
    madeWithLove: 'صنع بـ',

    // Additional
    appName: 'مدير الديون',
    downloadDesktop: 'تحميل نسخة سطح المكتب',
    downloadDesktopHint: 'تثبيت التطبيق على الكمبيوتر',
    desktopInstallBrowserHint: 'افتح الموقع عبر Chrome أو Edge ثم اختر تثبيت التطبيق من رمز التثبيت بجانب شريط العنوان أو من قائمة المتصفح.',
    quickActions: 'إجراءات سريعة',
    upcomingPayments: 'المدفوعات القادمة',
    results: 'نتائج',
    copyDetails: 'نسخ التفاصيل',
    copied: 'تم النسخ',
    refresh: 'تحديث',
    inactive: 'غير نشط',
    days: 'أيام'
  },

  fr: {
    // Auth
    login: 'Connexion',
    register: 'S\'inscrire',
    email: 'Email',
    password: 'Mot de passe',
    name: 'Nom',
    phone: 'Téléphone',
    confirmPassword: 'Confirmer le mot de passe',
    noAccount: 'Pas encore inscrit ? Inscrivez-vous',
    hasAccount: 'Déjà un compte ? Connectez-vous',
    loginTitle: 'Connectez-vous à votre compte',
    registerTitle: 'Créer un nouveau compte',
    logout: 'Déconnexion',
    forgotPassword: 'Mot de passe oublié ?',

    // Common
    welcome: 'Bienvenue',
    hello: 'Bonjour',
    howAreYou: 'Comment allez-vous ?',
    goodMorning: 'Bonjour',
    goodEvening: 'Bonsoir',
    thankYou: 'Merci',
    noProblem: 'Pas de problème',
    yes: 'Oui',
    no: 'Non',
    ok: 'OK',
    wait: 'Patientez',
    done: 'Terminé',
    loading: 'Chargement...',
    please: 'S\'il vous plaît',
    sorry: 'Désolé',

    // Navigation
    home: 'Accueil',
    debts: 'Dettes',
    settings: 'Paramètres',
    admin: 'Tableau de bord',
    back: 'Retour',
    next: 'Suivant',
    previous: 'Précédent',
    close: 'Fermer',

    // Debt Types
    debtOwedToMe: 'Dettes à recevoir',
    debtIOwe: 'Dettes à payer',
    owedToMe: 'À recevoir',
    iOwe: 'À payer',
    moneyOwed: 'Argent dû',
    moneyIOwe: 'Argent que je dois',

    // Debt Form
    addDebt: 'Ajouter une dette',
    editDebt: 'Modifier la dette',
    debtType: 'Type de dette',
    personName: 'Nom de la personne',
    amount: 'Montant',
    currency: 'Devise',
    dueDate: 'Date d\'échéance',
    notes: 'Notes',
    save: 'Enregistrer',
    cancel: 'Annuler',
    delete: 'Supprimer',
    confirm: 'Confirmer',

    // Status
    pending: 'En attente',
    paid: 'Payée',
    partiallyPaid: 'Partiellement payée',
    overdue: 'En retard',
    status: 'Statut',

    // Statistics
    totalOwedToMe: 'Total à recevoir',
    totalIOwe: 'Total à payer',
    paidDebts: 'Dettes payées',
    pendingDebts: 'Dettes en attente',
    statistics: 'Statistiques',
    recentActivity: 'Activité récente',
    totalDebts: 'Total des dettes',

    // Person
    people: 'Personnes',
    addPerson: 'Ajouter une personne',
    editPerson: 'Modifier la personne',
    deletePerson: 'Supprimer la personne',
    personDetails: 'Détails de la personne',
    contactPerson: 'Contacter',

    // Settings
    appearance: 'Apparence',
    darkMode: 'Mode sombre',
    lightMode: 'Mode clair',
    language: 'Langue',
    notifications: 'Notifications',
    whatsappReminder: 'Rappel WhatsApp',
    enableNotifications: 'Activer les notifications',
    enableWhatsapp: 'Activer les rappels WhatsApp',
    clearData: 'Effacer les données',
    accountSettings: 'Paramètres du compte',

    // Languages
    arabic: 'Arabe',
    french: 'Français',
    english: 'Anglais',

    // Admin
    users: 'Utilisateurs',
    userEmail: 'Email',
    registrationDate: 'Date d\'inscription',
    activate: 'Activer',
    deactivate: 'Désactiver',
    deleteUser: 'Supprimer l\'utilisateur',
    totalUsers: 'Total utilisateurs',
    activeUsers: 'Utilisateurs actifs',
    manageUsers: 'Gérer les utilisateurs',

    // Messages
    debtAdded: 'Dette ajoutée avec succès',
    debtUpdated: 'Dette mise à jour',
    debtDeleted: 'Dette supprimée',
    loginSuccess: 'Connexion réussie',
    logoutSuccess: 'Déconnexion réussie',
    registerSuccess: 'Inscription réussie',
    error: 'Erreur',
    success: 'Succès',
    confirmDelete: 'Confirmer la suppression ? Cette action est irréversible',
    noDebts: 'Aucune dette',
    noUsers: 'Aucun utilisateur',
    loadingData: 'Chargement...',
    savingData: 'Enregistrement...',
    networkError: 'Erreur réseau - Vérifiez votre connexion',
    invalidCredentials: 'Identifiants incorrects',
    emailExists: 'Email déjà utilisé',

    // Currency
    dzd: 'Dinar algérien',
    eur: 'Euro',
    usd: 'Dollar',

    // Notifications
    paymentReminder: 'Rappel de paiement',
    dueToday: 'Échéance aujourd\'hui',
    dueTomorrow: 'Échéance demain',
    overdueNotice: 'Dette en retard',
    notificationSent: 'Notification envoyée',

    // WhatsApp
    whatsappReminderTitle: 'Rappel de dette',
    whatsappGreeting: 'Bonjour,',
    whatsappBody: 'Je vous rappelle cette dette:',
    whatsappClosing: 'Merci',

    // Dates
    today: 'Aujourd\'hui',
    tomorrow: 'Demain',
    yesterday: 'Hier',
    thisWeek: 'Cette semaine',
    thisMonth: 'Ce mois',
    daysLeft: 'jours restants',
    daysOverdue: 'jours de retard',

    // Actions
    call: 'Appeler',
    sendMessage: 'Envoyer un message',
    markAsPaid: 'Marquer comme payée',
    markAsPending: 'Marquer en attente',
    editAmount: 'Modifier le montant',
    addNote: 'Ajouter une note',
    viewDetails: 'Voir les détails',
    share: 'Partager',

    // Filter
    search: 'Rechercher',
    searchPlaceholder: 'Rechercher par nom...',
    filter: 'Filtrer',
    filterByType: 'Filtrer par type',
    filterByStatus: 'Filtrer par statut',
    sortBy: 'Trier par',
    sortNewest: 'Plus récent',
    sortOldest: 'Plus ancien',
    sortAmount: 'Par montant',
    sortDueDate: 'Par date d\'échéance',

    // Misc
    version: 'Version',
    appInfo: 'À propos',
    contactSupport: 'Support',
    privacyPolicy: 'Confidentialité',
    termsOfService: 'Conditions',
    madeWithLove: 'Fait avec',
    appName: 'Gestionnaire de Dettes',
    downloadDesktop: 'Télécharger la version bureau',
    downloadDesktopHint: 'Installer l’application sur l’ordinateur',
    desktopInstallBrowserHint: 'Ouvrez le site avec Chrome ou Edge, puis choisissez Installer l’application depuis l’icône d’installation ou le menu du navigateur.',
    quickActions: 'Actions rapides',
    upcomingPayments: 'Paiements à venir',
    results: 'résultats',
    copyDetails: 'Copier les détails',
    copied: 'Copié',
    refresh: 'Actualiser',
    inactive: 'Inactif',
    days: 'jours'
  },

  en: {
    // Auth
    login: 'Login',
    register: 'Sign Up',
    email: 'Email',
    password: 'Password',
    name: 'Name',
    phone: 'Phone',
    confirmPassword: 'Confirm password',
    noAccount: 'No account? Sign up',
    hasAccount: 'Have an account? Login',
    loginTitle: 'Sign in to your account',
    registerTitle: 'Create new account',
    logout: 'Logout',
    forgotPassword: 'Forgot password?',

    // Common
    welcome: 'Welcome',
    hello: 'Hello',
    howAreYou: 'How are you?',
    goodMorning: 'Good morning',
    goodEvening: 'Good evening',
    thankYou: 'Thank you',
    noProblem: 'No problem',
    yes: 'Yes',
    no: 'No',
    ok: 'OK',
    wait: 'Please wait',
    done: 'Done',
    loading: 'Loading...',
    please: 'Please',
    sorry: 'Sorry',

    // Navigation
    home: 'Home',
    debts: 'Debts',
    settings: 'Settings',
    admin: 'Dashboard',
    back: 'Back',
    next: 'Next',
    previous: 'Previous',
    close: 'Close',

    // Debt Types
    debtOwedToMe: 'Money owed to me',
    debtIOwe: 'Money I owe',
    owedToMe: 'Owed to me',
    iOwe: 'I owe',
    moneyOwed: 'Money owed',
    moneyIOwe: 'Money I owe others',

    // Debt Form
    addDebt: 'Add debt',
    editDebt: 'Edit debt',
    debtType: 'Debt type',
    personName: 'Person name',
    amount: 'Amount',
    currency: 'Currency',
    dueDate: 'Due date',
    notes: 'Notes',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    confirm: 'Confirm',

    // Status
    pending: 'Pending',
    paid: 'Paid',
    partiallyPaid: 'Partially paid',
    overdue: 'Overdue',
    status: 'Status',

    // Statistics
    totalOwedToMe: 'Total owed to me',
    totalIOwe: 'Total I owe',
    paidDebts: 'Paid debts',
    pendingDebts: 'Pending debts',
    statistics: 'Statistics',
    recentActivity: 'Recent activity',
    totalDebts: 'Total debts',

    // Person
    people: 'People',
    addPerson: 'Add person',
    editPerson: 'Edit person',
    deletePerson: 'Delete person',
    personDetails: 'Person details',
    contactPerson: 'Contact',

    // Settings
    appearance: 'Appearance',
    darkMode: 'Dark mode',
    lightMode: 'Light mode',
    language: 'Language',
    notifications: 'Notifications',
    whatsappReminder: 'WhatsApp reminder',
    enableNotifications: 'Enable notifications',
    enableWhatsapp: 'Enable WhatsApp reminders',
    clearData: 'Clear all data',
    accountSettings: 'Account settings',

    // Languages
    arabic: 'Arabic',
    french: 'French',
    english: 'English',

    // Admin
    users: 'Users',
    userEmail: 'Email',
    registrationDate: 'Registration date',
    activate: 'Activate',
    deactivate: 'Deactivate',
    deleteUser: 'Delete user',
    totalUsers: 'Total users',
    activeUsers: 'Active users',
    manageUsers: 'Manage users',

    // Messages
    debtAdded: 'Debt added successfully',
    debtUpdated: 'Debt updated',
    debtDeleted: 'Debt deleted',
    loginSuccess: 'Login successful',
    logoutSuccess: 'Logout successful',
    registerSuccess: 'Registration successful',
    error: 'Error',
    success: 'Success',
    confirmDelete: 'Confirm deletion? This action cannot be undone',
    noDebts: 'No debts yet',
    noUsers: 'No users found',
    loadingData: 'Loading...',
    savingData: 'Saving...',
    networkError: 'Network error - Please check your connection',
    invalidCredentials: 'Invalid credentials',
    emailExists: 'Email already registered',

    // Currency
    dzd: 'Algerian Dinar',
    eur: 'Euro',
    usd: 'US Dollar',

    // Notifications
    paymentReminder: 'Payment reminder',
    dueToday: 'Due today',
    dueTomorrow: 'Due tomorrow',
    overdueNotice: 'Overdue debt',
    notificationSent: 'Notification sent',

    // WhatsApp
    whatsappReminderTitle: 'Debt reminder',
    whatsappGreeting: 'Hello,',
    whatsappBody: 'Reminding you about this debt:',
    whatsappClosing: 'Thanks',

    // Dates
    today: 'Today',
    tomorrow: 'Tomorrow',
    yesterday: 'Yesterday',
    thisWeek: 'This week',
    thisMonth: 'This month',
    daysLeft: 'days left',
    daysOverdue: 'days overdue',

    // Actions
    call: 'Call',
    sendMessage: 'Send message',
    markAsPaid: 'Mark as paid',
    markAsPending: 'Mark as pending',
    editAmount: 'Edit amount',
    addNote: 'Add note',
    viewDetails: 'View details',
    share: 'Share',

    // Filter
    search: 'Search',
    searchPlaceholder: 'Search by name...',
    filter: 'Filter',
    filterByType: 'Filter by type',
    filterByStatus: 'Filter by status',
    sortBy: 'Sort by',
    sortNewest: 'Newest',
    sortOldest: 'Oldest',
    sortAmount: 'By amount',
    sortDueDate: 'By due date',

    // Misc
    version: 'Version',
    appInfo: 'About',
    contactSupport: 'Support',
    privacyPolicy: 'Privacy',
    termsOfService: 'Terms',
    madeWithLove: 'Made with',
    appName: 'Debts Manager',
    downloadDesktop: 'Download desktop version',
    downloadDesktopHint: 'Install the app on this computer',
    desktopInstallBrowserHint: 'Open the site in Chrome or Edge, then choose Install app from the install icon near the address bar or the browser menu.',
    quickActions: 'Quick Actions',
    upcomingPayments: 'Upcoming Payments',
    results: 'results',
    copyDetails: 'Copy Details',
    copied: 'Copied',
    refresh: 'Refresh',
    inactive: 'Inactive',
    days: 'days'
  }
};

export const currencies = ['DZD', 'EUR', 'USD'];
